...
func generateNameWithRng(rng *rand.Rand) string {
	isFemale := rng.Intn(2) == 0
	...
	lastName := identityLastNames[rng.Intn(len(identityLastNames))]
	if isFemale {
...
func generateName() string {
	return generateNameWithRng(rand.New(rand.NewSource(rand.Int63())))
}
