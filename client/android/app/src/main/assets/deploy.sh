#!/bin/bash
# ==============================================================================
#  NxiwNetwork VPN Server — Универсальный установщик для VPS
...
readonly LOG_FILE="/var/log/nxiw-install.log"
...
fw_add_mss_clamping() {
    ...
    case "$FW_BACKEND" in
        iptables)
            # Применяем правило ТОЛЬКО к нашей подсети NxiwNetwork
...
# ══════════════════════════════════════════════════════════════════════════════
#  NxiwNetwork VPN SERVER DEPLOYMENT
# ══════════════════════════════════════════════════════════════════════════════

# ─── Очистка старого NxiwNetwork ─────────────────────────────────────────────────────
nxiw_cleanup() {
    prog 0.05 "Очистка..."
    echo "🧹 Очистка старой установки NxiwNetwork..."

    # Защита SSH
    fw_add_input_tcp 22

    systemctl unmask nxiwnetwork 2>/dev/null || true
    systemctl stop nxiwnetwork 2>/dev/null || true
    systemctl disable nxiwnetwork 2>/dev/null || true
    rm -f /etc/systemd/system/nxiwnetwork.service 2>/dev/null || true
    systemctl daemon-reload 2>/dev/null || true
    pkill -9 -f nxiwnetwork-server 2>/dev/null || killall -9 nxiwnetwork-server 2>/dev/null || true

    # Удаляем только NxiwNetwork WireGuard интерфейс (wg0), НЕ трогаем другие
    ip link del wg0 2>/dev/null || true

    # Удаляем старые правила NAT для NxiwNetwork подсети
    for i in {1..5}; do
    ...
    rm -f /usr/local/bin/nxiwnetwork-server 2>/dev/null || true
    rm -rf /etc/wireguard/wg-keys.dat /etc/wireguard/passwords.json /etc/wireguard/server.log 2>/dev/null || true

    # Освобождаем ТОЛЬКО порты NxiwNetwork (56001 и 56000), НЕ трогаем 51820!
...
setup_sysctl() {
    ...
    cat > /etc/sysctl.d/99-nxiwnetwork.conf << 'SYSEOF'
...
SYSEOF
    ...
    sysctl -p /etc/sysctl.d/99-nxiwnetwork.conf >/dev/null 2>&1 || true
...
# ─── Установка бинарника nxiwnetwork-server ──────────────────────────────────────────
setup_nxiw_binary() {
    prog 0.60 "Бинарник..."
    echo "📦 Установка nxiwnetwork-server..."

    if [ -f /tmp/nxiwnetwork-server ]; then
        chmod +x /tmp/nxiwnetwork-server
        mv /tmp/nxiwnetwork-server /usr/local/bin/nxiwnetwork-server
        echo "✓ nxiwnetwork-server установлен"
    elif [ -f /usr/local/bin/nxiwnetwork-server ]; then
        echo "✓ nxiwnetwork-server уже установлен"
    else
        echo "⚠ nxiwnetwork-server не найден в /tmp/ — пропускаем"
        echo "  Загрузите бинарник вручную в /usr/local/bin/nxiwnetwork-server"
    fi

    mkdir -p /etc/wireguard
}

# ─── Systemd-сервис NxiwNetwork ─────────────────────────────────────────────────────
setup_nxiw_service() {
    prog 0.75 "Сервис..."
    echo "🔧 Создание systemd-сервиса NxiwNetwork..."

    cat > /etc/systemd/system/nxiwnetwork.service << NXIWNETSVC
[Unit]
Description=NxiwNetwork VPN Server
...
ExecStart=/usr/local/bin/nxiwnetwork-server -listen 0.0.0.0:${DTLS_PORT} -wg-port ${WG_PORT} -config-dir /etc/wireguard ${NXIW_ARGS:-}
Restart=always
RestartSec=5
LimitNOFILE=65535
...
NXIWNETSVC
    ...
    systemctl unmask nxiwnetwork >/dev/null 2>&1 || true
    systemctl enable nxiwnetwork >/dev/null 2>&1 || true
    echo "✓ Сервис nxiwnetwork.service создан и включён"
}

# ─── Запуск NxiwNetwork ─────────────────────────────────────────────────────────────
start_nxiw() {
    prog 0.90 "Запуск..."
    echo "🚀 Запуск NxiwNetwork VPN Server..."

    if [ ! -f /usr/local/bin/nxiwnetwork-server ]; then
        echo "⚠ nxiwnetwork-server не установлен — запуск пропущен"
        return 0
    fi

    systemctl restart nxiwnetwork

    sleep 2
    local status
    status=$(systemctl is-active nxiwnetwork 2>/dev/null || echo "unknown")
    ...
    if [ "$status" = "active" ]; then
        ...
        echo "⚠️ Сервис NxiwNetwork не запустился. Статус: $status"
        echo "   Последние логи:"
        journalctl -u nxiwnetwork -n 7 --no-pager 2>/dev/null | sed 's/^/   >> /'
    fi

    echo "   Логи:   journalctl -u nxiwnetwork -f"
    echo "   Статус: systemctl status nxiwnetwork"
    echo "══════════════════════════════════════════════════════════════"
    echo ""
}
...
# ─── Команда: uninstall ──────────────────────────────────────────────────────
do_uninstall() {
    log_step "Удаление NxiwNetwork..."

    systemctl stop nxiwnetwork 2>/dev/null || true
    systemctl disable nxiwnetwork 2>/dev/null || true
    rm -f /etc/systemd/system/nxiwnetwork.service
    systemctl daemon-reload

    ip link del wg0 2>/dev/null || true
    pkill -9 -f nxiwnetwork-server 2>/dev/null || true
    ...
    rm -f /usr/local/bin/nxiwnetwork-server
    rm -f /etc/sysctl.d/99-nxiwnetwork.conf
    sysctl --system >/dev/null 2>&1 || true

    log_info "NxiwNetwork полностью удалён."
}

# ─── Команда: status ─────────────────────────────────────────────────────────
do_status() {
    echo "Статус NxiwNetwork:"
    echo ""
    if systemctl is-active nxiwnetwork &>/dev/null; then
...
main() {
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║       NxiwNetwork VPN Server — Installer v${SCRIPT_VERSION}                    ║"
    ...
    echo "=== NxiwNetwork Installer v${SCRIPT_VERSION} — $(date) ===" >> "$LOG_FILE"
