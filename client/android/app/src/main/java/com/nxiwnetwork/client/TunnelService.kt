...
class TunnelService : Service() {
    private companion object {
        ...
        const val TUNNEL_NOTIFICATION_CHANNEL_ID = "nxiw_tunnel_v2"
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent.action) {
            ...
            "DEPLOY_CANCEL" -> {
                com.nxiwnetwork.client.NodesManager.writeError("[!] ❌ Установка отменена пользователем")
                com.nxiwnetwork.client.NodesManager.stopDeploy("error: Отменена пользователем")
    ...
    private fun acquireWifiLock() {
        ...
        wifiLock = wm.createWifiLock(mode, "nxiw:wifi_perf").apply { 
            setReferenceCounted(false)
            acquire() 
        }
    }
